...
export default function Dashboard() {
  ...
  const getData = async () => {
    const result = await axios.get(
      `${ENDPOINTS.GET.ITEMS.ISSUED}/${user_id}/${company_id}`
    );
    setData(result.data);
  };
  ...
  const sendData = async () => {
    ...
    data.append("item_id", issued.item_id);
    ...
    <>
      ...
      >
        ...
        >
          <div className="w-full flex items-start justify-between mb-3">
            <div className="w-full">
              <h1 className="font-semibold text-[#2E3192]">Issued</h1>
            </div>
            <div className="flex w-full justify-end">
              <div
                onClick={() => {
      ...
      <div className="flex flex-col gap-8 w-full h-screen">
        <div className="w-full flex flex-col">
          <h1 className="font-extrabold p-2 text-sm lg:text-lg ">ISSUED</h1>
          <form
